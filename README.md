...
Static snapshot (GitHub Pages): [https://jermhey.github.io/opaque-housing/](https://jermhey.github.io/opaque-housing/). The insight app is FastAPI + DuckDB (`uv run oh serve`, Fly.io): mix-adjusted NYC vs PHL, definition toggles, anonymous neighborhood HHI, and `/v1` OpenAPI (ADR 0011). It syncs allowlisted `data/published` from this repo on boot and after the monthly refresh job. It is not Evidence.dev and it is not a name or address search.
